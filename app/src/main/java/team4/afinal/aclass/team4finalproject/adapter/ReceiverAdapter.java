...
import team4.afinal.aclass.team4finalproject.Activity6;
import team4.afinal.aclass.team4finalproject.Activity7Modify;
import team4.afinal.aclass.team4finalproject.Activity8Modify;
...
public class ReceiverAdapter extends BaseAdapter {
    ...
    // 생성자
    public ReceiverAdapter(Context context, List<ReceiverBean> list) {
        mContext = context;
        mList = list;
    }
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        convertView = inflater.inflate(R.layout.view_receiver, null);

        final ReceiverBean bean = mList.get(position);

        //해당 ROW 의 데이터를 찾는 작업
        bean.position = position;

        //인플레이팅 된 뷰에서 ID 찾는작업
        ImageView imgReceiver = convertView.findViewById( R.id.imgReceiver );
        ...
        try {
            new DownImgTask(imgReceiver).execute(new URL(bean.imgUrl));
        } catch (Exception e){
            e.printStackTrace();
        }
        txtName.setText(bean.studentId);
        ...
        btnModify.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Activity1 ac1 = new Activity1();
                if(bean.studentId.equals(ac1.mLoginedInfoBean.num)){
                    Intent intent = new Intent(mContext, Activity8Modify.class);
